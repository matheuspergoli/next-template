export class Throttler<Key> {
	...
	public consume(key: Key): boolean {
		...
		const index = Math.min(counter.timeout, this.timeoutSeconds.length - 1)
		const allowed = now - counter.updatedAt >= (this.timeoutSeconds[index] ?? 0) * 1000
		if (!allowed) {
			return false
		}
...
export const globalBucket = new RefillingTokenBucket<string>(50, 10) // 50 requests per 10 seconds

export const globalGETRateLimit = ({ clientIP }: { clientIP: string }) => {
	if (!clientIP) {
		return false
	}
...
export const globalPOSTRateLimit = ({ clientIP }: { clientIP: string }) => {
	if (!clientIP) {
		return false
	}
